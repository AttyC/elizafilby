...
function hoot_get_loop_title() {
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$loop_title_suffix = ' ' . sprintf( __( '(Page %s)', 'dispatch' ), $paged );
...
function hoot_get_loop_description() {
	...
	elseif ( is_search() )
		$loop_desc = sprintf( __( 'You are browsing the search results for &#8220;%s&#8221;', 'dispatch' ), get_search_query() );
...
function hoot_search_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Search results for &#8220;%s&#8221;', 'dispatch' ), get_search_query() );
