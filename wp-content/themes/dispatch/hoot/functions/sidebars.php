...
function hoot_widgetized_template_register_sidebars() {
	...
	if ( current_theme_supports( 'hoot-widgetized-template' ) ) :
		...
		$defaults = array(
			'area_a' => __('Widgetized Template - Area A', 'dispatch'),
			'area_b' => __('Widgetized Template - Area B', 'dispatch'),
			'area_c' => __('Widgetized Template - Area C', 'dispatch'),
			'area_d_1' => __('Widgetized Template - Area D Left', 'dispatch'),
			'area_d_2' => __('Widgetized Template - Area D Right', 'dispatch'),
		);
		...
		foreach ( $defaults as $key => $name ) {
			hoot_register_sidebar(
				array(
					...
					'name'        => $name,
					'description' => __( 'You can order Widgetized Template areas in Theme Options page.', 'dispatch' )
				)
			);
		}
