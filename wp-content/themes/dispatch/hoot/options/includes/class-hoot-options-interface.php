	...
	static function hootoptions_intro() {
		...
		$buttons = apply_filters( 'hootoptions_intro_buttons', array(
			...
			'support' => array( 'text'   => __( 'Support Forums', 'dispatch'),
								...
								'icon'   => 'support' ),
			'twitter' => array( 'text'   => __( 'Twitter', 'dispatch'),
								'button' => 'secondary',
								'url'    => 'http://twitter.com/wphootcom',
								'icon'   => 'twitter' ),
			'fb'      => array( 'text'   => __( 'Facebook', 'dispatch'),
								'button' => 'secondary',
								'url'    => 'http://facebook.com/wphoot',
								'icon'   => 'facebook' ),
			'premium' => array( 'text'   => __( 'Go Premium', 'dispatch'),
								'button' => 'primary',
	...
	static function hootoptions_field( $prefix, $group_name='', $field, $val, $echo=true ) {
		...
		case 'typography':
			...
			if ( isset( $field['options'] ) ) {
				...
				$explain_value .= ' ' . __( 'Some Fonts do not support all styles (bold, lighter, italic) For such fonts "Normal" style will be used.', 'dispatch' );
			}
		...
		case "icon":
			...
			$output .= '<div id="' . esc_attr( $field_id_prefix . $field['id'] . '-icon-picked' ) . '" class="hoot-of-icon-picked"><i class="fa ' . esc_attr( $val ) . '"></i><span>' . __( 'Select Icon', 'dispatch' ) . '</span></div>';

			$output .= '<div id="' . esc_attr( $field_id_prefix . $field['id'] . '-icon-picker-box' ) . '" class="hoot-of-icon-picker-box">';

				$output .= '<div class="hoot-of-icon-picker-list"><i class="fa fa-ban hoot-of-icon-none" data-value="0" data-category=""><span>' . __( 'Remove Icon', 'dispatch' ) . '</span></i></div>';
		...
		case "group":
			...
			$settings_defaults = array(
				'add_button' => __( 'Add Another Entry', 'dispatch' ),
				'remove_button' => __( 'Remove Entry', 'dispatch' ),
				'repeatable' => false,
				'sortable' => false,
				'toggleview' => true );
			if ( !empty( $field['settings']['sortable'] ) ) // check if sorting is set and true
				$settings_defaults['title'] = __( 'Group', 'dispatch' );
